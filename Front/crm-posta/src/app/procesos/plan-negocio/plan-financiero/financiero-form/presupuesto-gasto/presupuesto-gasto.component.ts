...
export class PresupuestoGastoComponent {
          ...
          this.procesoService.procesosFindAll().subscribe(pro => {
            ...
            this.procesos.forEach(proceso => {
              if (proceso.canvasModel.client.id == this.cliente.id) {
               ...
               console.log(proceso);
                if(this.idEditar){
                 this.administrativo= this.proceso.businessPlanFinancial.gastoCosto.administrativo;
                 this.operativo=this.proceso.businessPlanFinancial.gastoCosto.operativo;
                 this.ventas=this.proceso.businessPlanFinancial.gastoCosto.comercialVentas;
  ...
  sacarAdministrativo(administrativo:Personal){
    this.administrativo.personal= this.administrativo.personal.filter(o=>o!=administrativo)
    this.administrativo.parafiscalesCalculos()
  }
  sacarOperativo(operativo:Personal){
  console.log(operativo);

this.operativo.personal= this.operativo.personal.filter(o=>o!=operativo)
this.operativo.parafiscalesCalculos()
  ...
  sacarOperativoCostos(operativoCosto:OtrosCostos){
    this.operativo.costos=this.operativo.costos.filter(f=>f!=operativoCosto);
...
this.ventas.costos= this.ventas.costos.filter(f=>f!=ventaCosto);
  ...
  sacarAdministrativoCostos(administrativoCosto:OtrosCostos){
    ...
    this.administrativo.costos= this.administrativo.costos.filter(f=>f!=administrativoCosto);
  }
